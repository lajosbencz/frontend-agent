...
defineProps<{
  page: PageCollectionItemBase & { title: string }
  crumbTo: string
  crumbLabel: string
}>()
</script>
...
<template>
  <div class="wrap pt-6 pb-[72px]">
    <nav class="mx-auto mb-6 max-w-[700px] font-mono text-[11px] text-text-muted">
      <NuxtLink :to="crumbTo" class="text-text-muted hover:text-accent">{{ crumbLabel }}</NuxtLink> / <span class="text-text-2">{{ page.title }}</span>
    </nav>
